# 🌍 Eco-Monitor : Pipeline de Qualité de l'Air Temps Réel
...
```mermaid
graph LR
    API[☁️ Open-Meteo API] -->|JSON| Producer[🐍 Python Producer]
    Producer -->|Push| Kafka[📨 Kafka Topic]
    Kafka -->|Consume| Consumer[🐍 Python Consumer]
    Consumer -->|Insert| DB[(🐘 PostgreSQL)]
    
    subgraph Transformation & Qualité
    DB -->|Source| dbt[transformations dbt]
    Airflow[⏱️ Airflow] -->|Trigger| dbt
    end
    
    subgraph Exploitation
    dbt -->|Read| Grafana[📊 Grafana Dashboard]
    dbt -->|Read| Streamlit[🤖 Streamlit AI Assistant]
    end
```
...
Une vue instantanée des dernières mesures disponibles pour permettre aux utilisateurs de connaître la qualité de l'air à l'instant T.
